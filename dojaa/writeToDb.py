...
Usage:
    python writeToDb.py --file data.json
...
# ---------------------------------------------------------------------------
# Upsert asset → returns asset_id
# ---------------------------------------------------------------------------

def upsert_asset(cur, ip: str) -> int:
    """
    Insert a new asset row if the IP doesn't exist yet.
...
def insert_shodan_risk(cur, asset_id: int, record: dict):
    cur.execute(
        ...
        (
            ...
            record.get("severity"),
            record.get("issues"),          # stored as text[] — list passed directly
...
def process_shodan(cur, records: list):
    ...
    for record in records:
        ip = record.get("ip")
        if not ip:
            print("  WARNING: skipping shodan record with no IP")
            continue
...
def insert_ssl_tls(cur, asset_id: int, record: dict):
    cur.execute(
        ...
        (
            ...
            record.get("issuer"),
            record.get("subject"),
            record.get("expiry"),           # date string e.g. "2026-06-30"
            record.get("days_left"),
            record.get("san"),              # list → text[]
            ...
            record.get("san_clean"),        # list → text[]
            record.get("trust"),
        ),
    )
...
def process_ssl_tls(cur, records: list):
    ...
    for record in records:
        ip = record.get("ip")
        if not ip:
            print("  WARNING: skipping ssl_tls record with no IP")
            continue

        asset_id = upsert_asset(cur, ip)
        insert_ssl_tls(cur, asset_id, record)

    print(f"  SSL/TLS done.")
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                if shodan_records:
                    ...
                    print("  No shodan records found.")

                if ssl_tls_records:
                    process_ssl_tls(cur, ssl_tls_records)
                else:
                    print("  No ssl_tls records found.")

                if censys_records:
                    print(f"  WARNING: {len(censys_records)} censys records found but censys tables are not yet schemed — skipping.")
                else:
                    print("  No censys records (expected).")
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--file", required=True, help=f"Path to the JSON file (default: data.json next to this script)")
